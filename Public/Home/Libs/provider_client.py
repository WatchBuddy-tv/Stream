...
class RemoteProviderClient:
    ...
    async def _get(self, endpoint: str, params: dict[str, object] | None = None) -> object:
        try:
    ...
    async def get_schema(self) -> dict[str, object]:
    ...
    async def get_plugins(self) -> list[dict[str, object]]:
        res = await self._get("/api/v1/get_all_plugins")
        return res if isinstance(res, list) else []

    async def get_plugin(self, plugin_name: str) -> dict[str, object] | None:
        return await self._get("/api/v1/get_plugin", params={"plugin" : plugin_name})

    async def get_main_page(self, plugin_name: str, url: str, page: int = 1, category: str = "") -> list[dict[str, object]]:
    ...
    async def search(self, plugin_name: str, query: str, page: int = 1) -> list[dict[str, object]]:
    ...
    async def load_item(self, plugin_name: str, url: str) -> dict[str, object]:
    ...
    async def load_links(self, plugin_name: str, url: str) -> list[dict[str, object]]:
