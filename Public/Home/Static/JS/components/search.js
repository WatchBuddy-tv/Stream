...
class GlobalSearch {
    ...
    getEnabledPlugins() {
        const enabled = this.allPlugins.filter((p) => this.isPluginEnabled(p.name));

        // Ana arama, ada/dile göre filtrelenmiş eklenti ızgarası aktifse
        // sadece o alt kümede arasın (settings-drawer.js'in yayınladığı filtre).
        const gridFilter = window.__pluginGridFilter;
        const visibleNames = gridFilter && typeof gridFilter.getVisibleNames === 'function'
            ? gridFilter.getVisibleNames()
            : null;

        return visibleNames ? enabled.filter((p) => visibleNames.has(p.name)) : enabled;
    }
        ...
        const runNext = async () => {
            ...
            const searchTask = this.searchInPlugin(plugin.name, query, this.fetchHelper, { abortPrevious: false })
                ...
                .catch(error => {
                    ...
                    if (this.currentSearch !== query) return;
                    // Provider bloklu/rate-limitli (403 veya 429): kalan eklentilere istek atmayı anında durdur,
                    // devam eden istekleri iptal et ve henüz başlamamış olanları temizle.
                    if (error.providerBlocked) {
                        ...
                        this.showStatus(error.statusCode === 429 ? 'İstek limiti aşıldı (429), arama durduruldu.' : 'Erişim engellendi (403), arama durduruldu.', 'error');
                    }
                    completedSearches++;
                    this.searchResultsByPlugin.delete(plugin.name);
                    this.pendingPlugins.delete(plugin.name);
                    this.renderRankedResults(query, { showPending: completedSearches < this.plugins.length && !this.providerBlocked });
                    this.updateSearchStatus(completedSearches, this.plugins.length, totalResults);
                })
                .finally(() => {
    ...
    async searchInPlugin(pluginName, query, fetchHelper = null, fetchConfig = {}) {
        try {
            ...
            if (providerUrl) {
                ...
                url = `${base}/api/v1/search?query=${encodeURIComponent(query)}&plugin=${encodeURIComponent(pluginName)}`;
            } else {
                url = `/api/v1/search?query=${encodeURIComponent(query)}&plugin=${encodeURIComponent(pluginName)}`;
            }
            ...
            if (data?.provider_error && (data.provider_error.status_code === 403 || data.provider_error.status_code === 429)) {
                const error = new Error(`Provider HTTP ${data.provider_error.status_code}`);
                error.providerBlocked = true;
                error.statusCode      = data.provider_error.status_code;
                throw error;
            }
            return data.result || [];
        } catch (error) {
