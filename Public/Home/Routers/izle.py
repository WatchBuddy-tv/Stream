...
async def izle(
    ...
    # "baslik" bir sonraki bölüm linkine bu sayfadaki (zaten süslenmiş) başlıktan
    # üretilip taşınıyor — süsü tekrar süslemeden önce her zaman '‖' öncesindeki
    # temiz seri adına geri dön (BuddyFlutter next_episode_overlay ile aynı desen).
    NEXT_EP_TITLE_SEP = " ‖ "
    clean_baslik      = baslik.split(NEXT_EP_TITLE_SEP, 1)[0].strip() if NEXT_EP_TITLE_SEP in baslik else baslik

    full_baslik = clean_baslik
    if season and episode:
        full_baslik += f"{NEXT_EP_TITLE_SEP}S{season} E{episode}"
        if bolum_adi:
